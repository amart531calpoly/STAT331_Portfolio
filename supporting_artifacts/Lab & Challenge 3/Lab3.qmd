...
```{r}
age_demographic_clean_hiphop <- hiphop |> 
  distinct(subj, .keep_all = TRUE) |> 
  summarise(mean_age = mean(age))
age_demographic_clean_hiphop
```
...
ethnic_demographic_clean_hiphop <- hiphop |> 
  distinct(subj, .keep_all = TRUE) |> 
  group_by(ethnic) |> 
  count(ethnic)
ethnic_demographic_clean_hiphop
```
